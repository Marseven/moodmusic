...
class ArtistSearchSuggestionsController extends BaseController
{
    public function index()
    {
        ...
        $shouldListAll = $user->hasPermission('music.update') ||
            $user->getRestrictionValue('music.create', 'artist_selection') ||
            request()->has('listAll');

        $builder = $shouldListAll ?
            Artist::query() :
            $user->artists();

        $artists = $builder->where('name', 'like', $query.'%')->limit($limit)->get(['artists.id', 'name', 'image_small']);

        $alreadyHasCurrentUser = $artists->first(function(Artist $artist) {
            return $artist->pivot && $artist->pivot->user_id === Auth::user()->id;
        });
        if ( ! $alreadyHasCurrentUser) {
            $artists->prepend((object) [
                'id' => 'CURRENT_USER',
