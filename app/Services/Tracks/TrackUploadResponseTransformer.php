...
class TrackUploadResponseTransformer
{
    ...
    public function transform(array $response): array
    {
        ...
        $autoMatch = Auth::user()->hasPermission('music.update') || Auth::user()->getRestrictionValue('music.create', 'artist_selection');

        $getID3 = new getID3;

        if ($upload = $this->request->file('file')) {
            $metadata = $getID3->analyze(
                $upload->getPathname(), $fileEntry->file_size, $fileEntry->name
            );
        } else {
            $metadata = $getID3->analyze(
        ...
        }, Arr::except(Arr::get($metadata, 'comments', []), ['music_cd_identifier', 'text']));
        ...
        if (isset($normalizedMetadata['unsynchronised_lyric'])) {
            $lyric = preg_replace('/[\n]/', '<br>', $normalizedMetadata['unsynchronised_lyric']);
        ...
        if (isset($normalizedMetadata['album'])) {
            ...
            if ($autoMatch && isset($normalizedMetadata['artist'])) {
                $album = $normalizedMetadata['artist']->albums()->where('name', $normalizedMetadata['album_name'])->first();
                if ( ! $album) {
    ...
    private function storeMetadataPicture($normalizedMetadata)
    {
        ...
        $fileData = [
            ...
            'file_size' => $normalizedMetadata['picture']['datalength'] ?? strlen($normalizedMetadata['picture']['data']),
