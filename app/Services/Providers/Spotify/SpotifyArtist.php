...
class SpotifyArtist {
    ...
    public function getContent(Artist $artist, array $options = []): ?array {
        if ($artist->spotify_id) {
            $spotifyArtist = $this->httpClient->get("artists/{$artist->spotify_id}");
        }

        // if couldn't find artist, bail
        if ( ! isset($spotifyArtist)) return null;
       ...
       $response = ['mainInfo' => $mainInfo, 'genres' => $spotifyArtist['genres']];

       if (Arr::get($options, 'importAlbums', true)) {
           $partialAlbums = $this->httpClient->get("artists/{$artist->spotify_id}/albums?offset=0&limit=50&album_type=album,single");
