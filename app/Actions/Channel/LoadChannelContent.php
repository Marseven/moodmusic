...
{
    public function execute(Channel $channel, array $params = [], Channel $parent = null): ?AbstractPaginator
    {
        $params['perPage'] = $params['perPage'] ?? 50;
        $params['order'] = $params['order'] ?? Arr::get($channel->config, 'contentOrder');
        ...
        if ($paginator = $this->maybeLoadViaQuery($channel, $params)) {
            ...
            return Cache::remember("channels.$channel->id.$paramsKey", 1440, function() use($channel, $params, $parent) {
                return $this->load($channel, $params);
            });
        }
    }

    private function maybeLoadViaQuery(Channel $channel, array $params): ?AbstractPaginator
    {
       if (Arr::get($channel->config, 'contentType') === 'listAll') {
           switch (Arr::get($channel->config, 'contentModel')) {
               case Artist::MODEL_TYPE:
                   return app(PaginateArtists::class)->execute($params, $channel->genre);
               case Album::MODEL_TYPE:
                   return app(PaginateAlbums::class)->execute($params, $channel->genre);
               case Track::MODEL_TYPE:
                   return app(PaginateTracks::class)->execute($params, $channel->genre);
               ...
               case Playlist::MODEL_TYPE:
                   ...
                   return app(PaginatePlaylists::class)->execute($params, $builder);
           }
       }
       return null;
    }
    ...
    private function load(Channel $channel, array $params): ?AbstractPaginator
    {
        ...
        if ($modelType = Arr::get($channel, 'config.contentModel')) {
            ...
            if (Arr::get($params, 'simplePagination')) {
                $items = $builder->limit($params['perPage'])->orderBy('channelables.order')->get();
                $pagination = new SimplePaginator($items, $params['perPage'], 1);
            } else {
            ...
            $items->transform(function(Model $model) use($channel, $params) {
                ...
                if ($model instanceof Channel) {
                    ...
                    $model->loadContent(array_merge($params, ['perPage' => 10, 'simplePagination' => true, 'order' => null]), $channel);
                }
                return $model;
            });
        ...
        // otherwise will need to load pivot table first and then one query per model type
            ...
            $newCollection = $pagination
                ...
                ->map(function(Collection $channelableGroup, string $channelableModel) {
                    return $this->getQueryBuilderFor(app($channelableModel)::MODEL_TYPE)
                        ->whereIn('id', $channelableGroup->pluck('channelable_id'))
                        ->get()
                        ->map(function(Model $model) use($channelableGroup) {
                            $channelable = $channelableGroup->first(function($channelable) use($model) {
                                return (int) $channelable->channelable_id === $model['id'] && $channelable->channelable_type === get_class($model);
                            });
                            $model['channelable_id'] = $channelable->id;
                            $model['channelable_order'] = $channelable->order;
                            return $model;
                        });
                })->flatten(1)->sortBy('channelable_order')->values();
     ...
     * @return Builder
     */
    private function getQueryBuilderFor(string $modelType, Channel $channel = null)
    {
        switch($modelType) {
            case Track::MODEL_TYPE:
                return ($channel ? $channel->tracks() : app(Track::class))
                    ...
                    ->withCount('plays');
            case Album::MODEL_TYPE:
                return ($channel ? $channel->albums() : app(Album::class))
                    ->with('artists');
            case Artist::MODEL_TYPE:
                return ($channel ? $channel->artists() : app(Artist::class))
                    ->select(['name', 'artists.id', 'image_small']);
            case User::MODEL_TYPE:
                return ($channel ? $channel->users() : app(User::class))
                    ->select(['users.id', 'email', 'first_name', 'last_name', 'username', 'avatar']);
            case Genre::MODEL_TYPE:
                return $channel ? $channel->genres() :  app(Genre::class);
            case Playlist::MODEL_TYPE:
                return ($channel ? $channel->playlists() : app(Playlist::class))
                    ->with('editors');
    ...
    private function maybeConnectToGenre(Channel $channel, array $params = [], Channel $parent = null)
    {
        $channelToCheck = $parent ?? $channel;
        if ( ! Arr::get($params, 'forAdmin') && Arr::get($channelToCheck->config, 'connectToGenreViaUrl')) {
            $filter = Arr::get($params, 'filter');
            if ( ! $filter) abort(404);
            $genre = $parent->genre ?? Genre::whereName($filter)->select(['id', 'name', 'display_name'])->firstOrFail();
            $channel->setAttribute('genre', $genre);

            $channel->name = app(ReplacePlaceholders::class)
                ->execute($channel->name, ['channel' => $channel]) ?: $channel->name;

            if ( ! $parent && app(Settings::class)->get('artist_provider') !== 'local') {
                $channel->config = array_merge($channel->config, [
                    'actions' => [
                        ['icon' => 'antenna', 'tooltip' => 'Genre radio', 'route' => "/radio/genre/{$channel->genre->id}/{$channel->genre->name}"],
                    ]
                ]);
            }
        }
